...
- `apps/web/app/db.server.ts`: Prismaベースのデータベース接続
...
#### 2. **データフロー**
```
Shopify Admin → Cloudflare Workers (apps/web) → Supabase / Redis
                         ↓
                Separate OCR/Sync backend
```

#### 3. **デプロイ方針**
- **フロント本体**: Shopify Embedded App と軽量 API は `apps/web` に集約
- **重い処理**: OCR / PDF 処理 / 在庫同期は分離バックエンドへ逃がせる構成
- **意図**: 埋め込み UI の応答性と、重い業務処理の安定性を両立するため
#### 4. **ステータス管理**
- 6段階の入荷ステータス:
  1. SI発行済み
  2. 船積スケジュール確定
...
Gemini API → データ補完 → Supabase保存 → Shopify同期
```
...
- **フレームワーク**: [Remix](https://remix.run) / 移行先候補は React Router
...
- **AI補完**: [Gemini API](https://ai.google.dev/)
